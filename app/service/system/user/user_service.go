...
//根据主键查询用户信息
func SelectRecordById(id int64) (*userModel.SysUser, error) {
	entity := &userModel.SysUser{UserId: id}
...
func EditSave(req *userModel.EditReq, c *gin.Context) (int64, error) {
	user := &userModel.SysUser{UserId: req.UserId}
	ok, err := user.FindOne()
	if err != nil {
		return 0, err
	}
...
func SignIn(loginnName, password string) (*userModel.SysUser, error) {
	//查询用户信息
	user := userModel.SysUser{LoginName: loginnName}
	ok, err := user.FindOne()
...
//清空用户菜单缓存
func ClearMenuCache(user *userModel.SysUser) {
...
func CheckPassport(loginName string) bool {
	entity := userModel.SysUser{LoginName: loginName}
...
func CheckNickName(userName string) bool {
	entity := userModel.SysUser{UserName: userName}
	if ok, err := entity.FindOne(); err != nil || !ok {
		return false
	} else {
		return true
...
func ResetPassword(params *userModel.ResetPwdReq) (bool, error) {
	user := userModel.SysUser{UserId: params.UserId}
...
//校验密码是否正确
func CheckPassword(user *userModel.SysUser, password string) bool {
...
//根据登陆名查询用户信息
func SelectUserByLoginName(loginName string) (*userModel.SysUser, error) {
	return userModel.SelectUserByLoginName(loginName)
}

//根据手机号查询用户信息
func SelectUserByPhoneNumber(phonenumber string) (*userModel.SysUser, error) {
	return userModel.SelectUserByPhoneNumber(phonenumber)
}

// 查询已分配用户角色列表
func SelectAllocatedList(roleId int64, loginName, phonenumber string) ([]userModel.SysUser, error) {
	return userModel.SelectAllocatedList(roleId, loginName, phonenumber)
}

// 查询未分配用户角色列表
func SelectUnallocatedList(roleId int64, loginName, phonenumber string) ([]userModel.SysUser, error) {
