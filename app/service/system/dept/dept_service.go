...
import (
	...
	"rygo/app/model"
	deptModel "rygo/app/model/system/dept"
	"strings"
	"time"
...
func AddSave(req *deptModel.AddReq, c *gin.Context) (int64, error) {
	dept := new(deptModel.SysDept)
	parent := &deptModel.SysDept{DeptId: req.ParentId}
...
func EditSave(req *deptModel.EditReq, c *gin.Context) (int64, error) {
	dept := &deptModel.SysDept{DeptId: req.DeptId}
	ok, err := dept.FindOne()
	if err != nil || !ok {
		return 0, errors.New("数据不存在")
	}
	pdept := &deptModel.SysDept{DeptId: req.ParentId}
...
//根据分页查询部门管理数据
func SelectListAll(param *deptModel.SelectPageReq) ([]deptModel.SysDept, error) {
	if param == nil {
		return SelectDeptList(0, "", "", param.TenantId)
	} else {
		return SelectDeptList(param.ParentId, param.DeptName, param.Status, param.TenantId)
	}
}
...
//根据部门ID查询信息
func SelectDeptById(deptId int64) *deptModel.SysDeptExtend {
...
//根据ID查询所有子部门
func SelectChildrenDeptById(deptId int64) []*deptModel.SysDept {
	return deptModel.SelectChildrenDeptById(deptId)
}

//加载部门列表树
func SelectDeptTree(parentId int64, deptName, status string, tenantId int64) (*[]model.Ztree, error) {
	list, err := deptModel.SelectDeptList(parentId, deptName, status, tenantId)
	if err != nil {
		return nil, err
	}
...
//查询部门管理数据
func SelectDeptList(parentId int64, deptName, status string, tenantId int64) ([]deptModel.SysDept, error) {
	return deptModel.SelectDeptList(parentId, deptName, status, tenantId)
}

//根据角色ID查询部门（数据权限）
func RoleDeptTreeData(roleId int64, tenantId int64) (*[]model.Ztree, error) {
	var result *[]model.Ztree
	deptList, err := deptModel.SelectDeptList(0, "", "", tenantId)
	if err != nil {
		return nil, err
	}
...
//对象转部门树
func InitZtree(deptList *[]deptModel.SysDept, roleDeptList *[]string) *[]model.Ztree {
	var result []model.Ztree
	isCheck := false
