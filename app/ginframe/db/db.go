...
func Instance(driver ...string) *dbEngine {
	once.Do(func() {
		...
		if len(config.Database.Slave) == 0 {
			engine, err := xorm.NewEngine(config.Database.Driver, config.Database.Master)
			if err != nil {
			...
			master, err := xorm.NewEngine(config.Database.Driver, config.Database.Master)
			if err != nil {
				return
			}
			if cfg.Instance().Database.Debug {
			...
			slave, err := xorm.NewEngine(config.Database.Driver, config.Database.Slave)
			if err != nil {
				return
			}
