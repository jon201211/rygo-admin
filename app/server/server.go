...
func (s *Server) Template(path string) *Server {
	...
	s.Handler.HTMLRender = gintemplate.New(gintemplate.TemplateConfig{
		...
		Funcs: template.FuncMap{
			"hasPermi":          service.PermissionService.HasPermi,
			"getPermiButton":    service.PermissionService.GetPermiButton,
			"getDictLabel":      service.DictService.GetDictLabel,
			"getDictTypeSelect": service.DictService.GetDictTypeSelect,
			"getDictTypeRadio":  service.DictService.GetDictTypeRadio,
			"getDictTypeData":   service.DictService.GetDictTypeData,
			"OssUrl":            service.ConfigService.GetOssUrl,
			"addInt":            calc.AddInt,
		},
		DisableCache: true,
	})
